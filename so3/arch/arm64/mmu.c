...
static void alloc_init_l3(u64 *l0pgtable, addr_t addr, addr_t end, addr_t phys,
			  bool nocache, mmu_stage_t stage)
{
		...
		if (!*l2pte) {
			...
			*l2pte = __pa((addr_t)l3pgtable) &
			...
			if (stage == S1)
				...
				set_pte_table_S2(l2pte,
						 (nocache ? DCACHE_OFF :
							    DCACHE_WRITEALLOC));
#else
			set_pte_table(l2pte, (nocache ? DCACHE_OFF :
							DCACHE_WRITEALLOC));
#endif
...
#ifdef CONFIG_ARM64VT
		if (stage == S1)
			set_pte_page(l3pte, (nocache ? DCACHE_OFF :
						       DCACHE_WRITEALLOC));
		else
			set_pte_page_S2(l3pte, (nocache ? DCACHE_OFF :
							  DCACHE_WRITEALLOC));
#else
...
static void alloc_init_l2(u64 *l0pgtable, addr_t addr, addr_t end, addr_t phys,
			  bool nocache, mmu_stage_t stage)
{
	u64 *l1pte, *l2pte;
	u64 *l2pgtable;
...
#error "Wrong VA_BITS configuration."
		...
		if (!*l1pte) {
			...
			*l1pte = __pa((addr_t)l2pgtable) &
				 TTB_L1_TABLE_ADDR_MASK;
		...
		if (((addr | next | phys) & ~BLOCK_2M_MASK) == 0) {
			*l2pte = phys & TTB_L2_BLOCK_ADDR_MASK;
			...
			if (stage == S1)
				...
				set_pte_block_S2(l2pte,
						 (nocache ? DCACHE_OFF :
							    DCACHE_WRITEALLOC));
#else
			set_pte_block(l2pte, (nocache ? DCACHE_OFF :
							DCACHE_WRITEALLOC));
...
static void alloc_init_l1(u64 *l0pgtable, addr_t addr, addr_t end, addr_t phys,
			  bool nocache, mmu_stage_t stage)
{
	u64 *l0pte, *l1pte;
	u64 *l1pgtable;
		...
		if (((addr | next | phys) & ~BLOCK_1G_MASK) == 0) {
			*l1pte = phys & TTB_L1_BLOCK_ADDR_MASK;
			...
			if (stage == S1)
				...
				set_pte_block_S2(l1pte,
						 (nocache ? DCACHE_OFF :
							    DCACHE_WRITEALLOC));
#else
			set_pte_block(l1pte, (nocache ? DCACHE_OFF :
							DCACHE_WRITEALLOC));
 ...
 *
 */
void __create_mapping(void *pgtable, addr_t virt_base, addr_t phys_base,
		      size_t size, bool nocache, mmu_stage_t stage)
{
 ...
 */
static void __create_mapping(void *l1pgtable, addr_t virt_base,
			     addr_t phys_base, size_t size, bool nocache,
			     mmu_stage_t stage)
{
	...
	mmu_page_table_flush((addr_t)l1pgtable,
			     (addr_t)(l1pgtable + TTB_L1_ENTRIES));
}
...
void create_mapping(void *pgtable, addr_t virt_base, addr_t phys_base,
		    size_t size, bool nocache)
{
...
#ifdef CONFIG_VA_BITS_48
		__sys_root_pgtable[l0pte_index(mem_info.phys_base)] =
			(u64)__sys_idmap_l1pgtable & TTB_L0_TABLE_ADDR_MASK;
		set_pte_table(
			&__sys_root_pgtable[l0pte_index(mem_info.phys_base)],
			DCACHE_WRITEALLOC);

		__sys_idmap_l1pgtable[l1pte_index(mem_info.phys_base)] =
			mem_info.phys_base & TTB_L1_BLOCK_ADDR_MASK;
		set_pte_block(
			&__sys_idmap_l1pgtable[l1pte_index(mem_info.phys_base)],
			DCACHE_WRITEALLOC);

#elif CONFIG_VA_BITS_39
	__sys_root_pgtable[l1pte_index(mem_info.phys_base)] =
		mem_info.phys_base & TTB_L1_BLOCK_ADDR_MASK;
	set_pte_block(&__sys_root_pgtable[l1pte_index(mem_info.phys_base)],
		      DCACHE_WRITEALLOC);
#else
#error "Wrong VA_BITS configuration."
#endif
		...
		__sys_root_pgtable[l0pte_index(CONFIG_KERNEL_VADDR)] =
			(u64)__sys_linearmap_l1pgtable & TTB_L0_TABLE_ADDR_MASK;
		set_pte_table(
			&__sys_root_pgtable[l0pte_index(CONFIG_KERNEL_VADDR)],
			DCACHE_WRITEALLOC);

		__sys_linearmap_l1pgtable[l1pte_index(CONFIG_KERNEL_VADDR)] =
			(u64)__sys_linearmap_l2pgtable & TTB_L1_TABLE_ADDR_MASK;
		set_pte_table(&__sys_linearmap_l1pgtable[l1pte_index(
				      CONFIG_KERNEL_VADDR)],
			      DCACHE_WRITEALLOC);
		 ...
		 */
		for (i = 0; i < 64; i++) {
			__sys_linearmap_l2pgtable[l2pte_index(
				CONFIG_KERNEL_VADDR + i * SZ_2M)] =
				(mem_info.phys_base + i * SZ_2M) &
				TTB_L2_BLOCK_ADDR_MASK;
			set_pte_block(&__sys_linearmap_l2pgtable[l2pte_index(
					      CONFIG_KERNEL_VADDR + i * SZ_2M)],
				      DCACHE_WRITEALLOC);
		}
#elif CONFIG_VA_BITS_39
	__sys_root_pgtable[l1pte_index(CONFIG_KERNEL_VADDR)] =
		(u64)__sys_linearmap_l2pgtable & TTB_L1_TABLE_ADDR_MASK;
	set_pte_table(&__sys_root_pgtable[l1pte_index(CONFIG_KERNEL_VADDR)],
		      DCACHE_WRITEALLOC);
	...
	/* Set up a 128 MB linear mapping to progress with the bootstrap code
		 ...
		 */
	for (i = 0; i < 64; i++) {
		__sys_linearmap_l2pgtable[l2pte_index(CONFIG_KERNEL_VADDR +
						      i * SZ_2M)] =
			(mem_info.phys_base + i * SZ_2M) &
			TTB_L2_BLOCK_ADDR_MASK;
		set_pte_block(&__sys_linearmap_l2pgtable[l2pte_index(
...
#ifdef CONFIG_VA_BITS_48
		__sys_idmap_l1pgtable[l1pte_index(CONFIG_UART_LL_PADDR)] =
			CONFIG_UART_LL_PADDR & TTB_L1_BLOCK_ADDR_MASK;
		set_pte_block(&__sys_idmap_l1pgtable[l1pte_index(
				      CONFIG_UART_LL_PADDR)],
			      DCACHE_OFF);
#elif CONFIG_VA_BITS_39
	__sys_root_pgtable[l1pte_index(CONFIG_UART_LL_PADDR)] =
		CONFIG_UART_LL_PADDR & TTB_L1_BLOCK_ADDR_MASK;
	set_pte_block(&__sys_root_pgtable[l1pte_index(CONFIG_UART_LL_PADDR)],
		      DCACHE_OFF);
#else
#error "Wrong VA_BITS configuration."
#endif
 ...
 */
void duplicate_pgtable_entry(u64 *from, u64 *to, int level, u64 vaddr,
			     pcb_t *pcb_to)
{
	u64 *__from, *__to;
	u64 mask;
	...
	if (level < 3) {
		...
		for (i = 0; i < ttb_entries; i++) {
			if (from[i]) {
				...
				duplicate_pgtable_entry(__from, __to, level + 1,
							__vaddr, pcb_to);
			}
		}
		...
		for (i = 0; i < TTB_L3_ENTRIES; i++) {
			if (from[i]) {
				...
				to[i] = (from[i] & ~TTB_L3_PAGE_ADDR_MASK) |
					(paddr_to & TTB_L3_PAGE_ADDR_MASK);

				/* Add the new page to the process list */
				add_page_to_proc(pcb_to, (page_t *)phys_to_page(
								 paddr_to));

				create_mapping(NULL, FIXMAP_MAPPING, paddr_to,
					       PAGE_SIZE, false);

				memcpy((void *)FIXMAP_MAPPING, (void *)__vaddr,
				       PAGE_SIZE);
			}
		}
...
#ifdef CONFIG_VA_BITS_48
	duplicate_pgtable_entry((u64 *)from->pgtable, (u64 *)to->pgtable, 0, 0,
				to);
#elif CONFIG_VA_BITS_39
	duplicate_pgtable_entry((u64 *)from->pgtable, (u64 *)to->pgtable, 1, 0,
				to);
#else
#error "Wrong VA_BITS configuration."
#endif
...
void do_ipamap(void *pgtable, ipamap_t ipamap[], int nbelement)
{
	...
	for (i = 0; i < nbelement; i++)
		__create_mapping(pgtable, ipamap[i].ipa_addr,
				 ipamap[i].phys_addr, ipamap[i].size, true, S2);
}
