...
void elf_load_segments(elf_img_info_t *elf_img_info)
{
	...
	for (i = 0; i < elf_img_info->header->e_phnum; i++) {
		DBG("[0x%08x] vaddr: 0x%08x; paddr: 0x%08x; filesize: 0x%08x; memsize: 0x%08x flags: 0x%08x\n",
		    elf_img_info->segments[i].p_offset, elf_img_info->segments[i].p_vaddr, elf_img_info->segments[i].p_paddr,
		    elf_img_info->segments[i].p_filesz, elf_img_info->segments[i].p_memsz, elf_img_info->segments[i].p_flags);
	}
}
