...
void heap_init(void)
{
	...
	LOG_DEBUG("SO3: allocating a kernel heap of %d bytes at address %p.", quick_list->size, quick_list);

	LOG_TRACE("[list_init] List initialized. sizeof(mem_chunk_t) = %d bytes, sizeof(int) = %d bytes", sizeof(mem_chunk_t),
		  sizeof(int));
}
...
void dump_heap(const char *info)
{
	...
	while (list) {
		...
		while (chunk) {
			LOG_DEBUG("[%s - print_list] [%p] %d size = %d bytes, head = %p, next_list = %p, next_chunk = %p ---> ",
...
{
	LOG_TRACE("[%s - print_chunk] [%p] [%s] %d bytes, next_list = %p, next_chunk = %p", caller, chunk, name, chunk->size,
...
static void append_chunk(mem_chunk_t *chunk)
{
	...
	LOG_TRACE("[append_chunk] %p with size %d padding: %d", chunk, chunk->size, chunk->padding_bytes);
	...
	if (!quick_list) {
		...
		if (tmp_list != NULL) { /* We found a position in the list (sorted items) */
			...
			if (tmp_list->size == chunk->size) { /* Good, we have found the right size */
				LOG_TRACE("[append_chunk] found a list with the right size %d", chunk->size);
				tmp_chunk = tmp_list;
				...
				LOG_TRACE("[append_chunk] creating list of new size %d", chunk->size);
			...
			LOG_TRACE("[append_chunk] adding at the end of quick_list with new size %d", chunk->size);
...
recheck:
	...
	while (tmp_list) {
		...
		while (tmp_chunk) {
			...
			if ((char *) new_chunk + new_chunk->size + sizeof(mem_chunk_t) ==
				    (char *) tmp_chunk || /* adjacent chunk after */
			    (char *) tmp_chunk + tmp_chunk->size + sizeof(mem_chunk_t) ==
				    (char *) new_chunk) { /* adjacent chunk before */

				/* tmp_chunk is adjacent to new_chunk. Merge them */
				LOG_TRACE("[merge_chunks] merging new_chunk [%p] with tmp_chunk [%p]", new_chunk, tmp_chunk);
#ifdef DEBUG
				dump_heap(__func__);
#endif
				...
				if (new_chunk < tmp_chunk) {
					...
					new_chunk->size +=
						tmp_chunk->size + sizeof(mem_chunk_t); /* Recover space used by memchunk */
				...
				LOG_TRACE("[merge_chunks] tmp_chunk: %p new_chunk: %p merged_chunk: %p merged_chunk->size = %d",
...
static void *__malloc(size_t requested, unsigned int alignment)
{
	...
	*/
	...
	LOG_TRACE("[malloc] requested size = %d, mem_chunk_size = %d bytes", requested, sizeof(mem_chunk_t));
...
void free(void *ptr)
{
	...
	chunk = (mem_chunk_t *) ((char *) ptr - sizeof(mem_chunk_t));
...
void *realloc(void *__ptr, size_t __size)
{
	...
	LOG_TRACE("Copying a size of %d", ((__size < chunk->req_size) ? __size : chunk->req_size));
